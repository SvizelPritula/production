...
function getFallbackImage(name: string) {
    ...
    result += `<text x="50" y="50" dominant-baseline="middle" text-anchor="middle">${escapeXML(name)}</text>`;
    result += `</svg>`;
...
export async function loadAssets(): Promise<AssetStore> {
    const assetDirectory = path.join(require.main!.path, "../assets");
    var files = await fs.readdir(assetDirectory);

    var fileEntries = files.map(file => path.parse(file)).map(file => ({
        name: file.name,
        type: file.ext,
    ...
    var promises = fileEntries.map(file => fs.readFile(file.path, { encoding: "utf-8" }));
